...
<body class="bg-slate-50 text-slate-800 antialiased font-sans min-h-screen relative overflow-x-hidden">
        ...
        <div class="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
            <div>
                ...
                <p class="text-slate-300 text-sm sm:text-base mt-1 max-w-xl">
                    {{ __('Visualiza los plazos de cierre, organiza tus documentos y gestiona tu agenda personal de postulación en un solo lugar.') }}
                </p>
            </div>
            ...
            <div class="grid grid-cols-3 gap-3 sm:gap-4 shrink-0">
                <div class="bg-slate-800/80 backdrop-blur-md border border-slate-700/60 rounded-xl p-3 text-center min-w-[90px]">
                    ...
                    <span class="text-[11px] font-medium text-slate-400 uppercase tracking-wider">{{ __('Scholarships') }}</span>
                </div>
                <div class="bg-slate-800/80 backdrop-blur-md border border-slate-700/60 rounded-xl p-3 text-center min-w-[90px]">
                    <span id="stat-tareas-count" class="block text-2xl font-bold font-heading text-blue-400">0</span>
                    <span class="text-[11px] font-medium text-slate-400 uppercase tracking-wider">{{ __('Schedule') }}</span>
                </div>
                <div class="bg-slate-800/80 backdrop-blur-md border border-slate-700/60 rounded-xl p-3 text-center min-w-[90px]">
                    <span id="stat-urgentes-count" class="block text-2xl font-bold font-heading text-rose-400">0</span>
                    <span class="text-[11px] font-medium text-slate-400 uppercase tracking-wider">{{ __('Por cerrar') }}</span>
                </div>
            </div>
        </div>
    </section>
    ...
    <div class="max-w-7xl mx-auto p-4 md:p-6 lg:p-8 grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <aside class="lg:col-span-1 space-y-6">
            ...
            <button id="btn-nueva-tarea" class="w-full btn-ugf-primary font-bold py-3 px-4 rounded-xl text-sm flex items-center justify-center space-x-2 tracking-wide group">
                ...
                <span>{{ __('Agregar Tarea a Agenda') }}</span>
            </button>
            ...
            <div class="glass-card glass-card-hover p-5 rounded-2xl">
                <h3 class="font-bold text-slate-800 text-xs tracking-wider uppercase font-heading mb-4 flex items-center space-x-2">
                    ...
                    <span>{{ __('Estado de Convocatorias') }}</span>
                </h3>
                <div class="space-y-3 text-xs">
                    <div class="flex items-center space-x-3 p-2 rounded-lg hover:bg-slate-100/60 transition">
                        ...
                        <div class="flex-1 flex justify-between items-center">
                            <span class="font-semibold text-slate-700">{{ __('Más de 15 días') }}</span>
                    ...
                    <div class="flex items-center space-x-3 p-2 rounded-lg hover:bg-slate-100/60 transition">
                        ...
                        <div class="flex-1 flex justify-between items-center">
                            <span class="font-semibold text-slate-700">{{ __('Próximo a cerrar') }}</span>
                    ...
                    <div class="flex items-center space-x-3 p-2 rounded-lg hover:bg-slate-100/60 transition">
                        ...
                        <div class="flex-1 flex justify-between items-center">
                            <span class="font-semibold text-slate-700">{{ __('Últimos días') }}</span>
                    ...
                    <div class="flex items-center space-x-3 p-2 rounded-lg hover:bg-slate-100/60 transition">
                        ...
                        <div class="flex-1 flex justify-between items-center">
                            <span class="font-semibold text-slate-700">{{ __('Tarea Personal') }}</span>
                            <span class="text-[10px] text-blue-700 bg-blue-100 px-2 py-0.5 rounded-full font-bold">{{ __('Log') }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="glass-card glass-card-hover p-5 rounded-2xl">
                <h3 class="font-bold text-slate-800 text-xs tracking-wider uppercase font-heading mb-4 flex items-center space-x-2">
                    ...
                    <span>{{ __('Filtros de Beca') }}</span>
                </h3>
                <div class="space-y-3">
                    <div>
                        <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">{{ __('Nivel de Estudio') }}</label>
                        <div class="relative">
                            <select id="filtro-nivel" class="w-full text-xs font-semibold text-slate-700 bg-white border border-slate-200 rounded-xl p-3 pr-8 shadow-xs focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 outline-none appearance-none cursor-pointer transition">
                                ...
                                <option value="grado">{{ __('🎓 Pregrado / Licenciatura') }}</option>
                                <option value="maestria">{{ __('📜 Maestría / Posgrado') }}</option>
                                <option value="doctorado">{{ __('🔬 Doctorado / PhD') }}</option>
                            </select>
            ...
            <div class="bg-gradient-to-br from-indigo-900 to-slate-900 text-white p-5 rounded-2xl shadow-md border border-indigo-800/50">
                ...
                <p class="text-xs text-slate-300 leading-relaxed">
                    {{ __('Consulta el calendario semanalmente y') }} <strong class="text-yellow-300">{{ __('planifica tus actividades') }}</strong> {{ __('en la agenda para cumplir cada requisito a tiempo.') }}
                </p>
            </div>
    ...
    <div id="modal-beca" class="fixed inset-0 bg-slate-900/60 backdrop-blur-md hidden flex items-center justify-center p-4 z-50">
        <div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden transform transition-all animate-modal-in border border-slate-100">
            ...
            <div id="contenedor-beca" class="p-6 space-y-5 max-h-[60vh] overflow-y-auto custom-scrollbar hidden">
                <div>
                    <h4 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">{{ __('Descripción') }}</h4>
                    <p id="modal-descripcion" class="text-sm text-slate-600 bg-slate-50 p-3.5 rounded-xl border border-slate-100 leading-relaxed">{{ __('Sin información disponible.') }}</p>
                </div>

                <div>
                    <h4 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">{{ __('Checklist de Postulación') }}</h4>
            ...
            <form id="form-editar-tarea" onsubmit="return false;" class="p-6 space-y-4 hidden">
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">{{ __('Título de la Tarea') }}</label>
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">{{ __('Fecha de la Tarea') }}</label>
            ...
            <div class="p-4 bg-slate-50 border-t border-slate-100 flex justify-between items-center">
                ...
                <div class="flex space-x-2 ml-auto">
                    <button type="button" onclick="cerrarModal()" class="px-4 py-2 text-xs font-bold text-slate-600 hover:bg-slate-200 rounded-xl transition">{{ __('Cerrar') }}</button>
        ...
        <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-modal-in border border-slate-100">
            ...
            <div class="p-6 bg-gradient-to-r from-slate-900 to-indigo-950 text-white border-b border-slate-800 flex justify-between items-center">
                <div class="flex items-center space-x-2">
                    ...
                    <h3 class="text-lg font-bold font-heading text-white">{{ __('Nueva Tarea Personal') }}</h3>
                </div>
            ...
            <form id="form-crear-tarea" class="p-6 space-y-4">
                <div>
                    <label class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">{{ __('Título de la Tarea') }}</label>
                    <input type="text" id="tarea-titulo" required placeholder="{{ __('Ej. Solicitar récord de notas a la U') }}" class="w-full border-slate-200 rounded-xl p-3 border text-sm text-slate-800 focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 outline-none transition shadow-xs">
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">{{ __('Fecha Programada') }}</label>
                ...
                <div class="pt-3 flex justify-end space-x-2">
                    <button type="button" onclick="cerrarModalCrear()" class="px-4 py-2.5 text-xs font-bold text-slate-600 hover:bg-slate-100 rounded-xl transition">{{ __('Cancelar') }}</button>
                    <button type="submit" class="btn-ugf-primary px-5 py-2.5 text-xs font-bold rounded-xl transition">{{ __('Guardar Tarea') }}</button>
                </div>
            </form>
    ...
    <!-- Traducciones JS global para JavaScript -->
    <script>
        window.i18n = {
            agendaPersonal: @json(__('📌 Agenda Personal')),
            editarTareaPersonal: @json(__('Editar Tarea Personal')),
            sinInformacion: @json(__('Sin información disponible.')),
            noChecklist: @json(__('No hay requisitos en la lista de verificación para esta convocatoria.')),
            completarCampos: @json(__('Completa todos los campos')),
            confirmEliminar: @json(__('¿Estás seguro de eliminar esta tarea de tu agenda?')),
            tareaAgregada: @json(__('Tarea agregada exitosamente')),
            tareaActualizada: @json(__('Tarea actualizada exitosamente')),
            tareaEliminada: @json(__('Tarea eliminada')),
            errorGuardar: @json(__('Error al guardar la tarea')),
            errorActualizar: @json(__('Error al actualizar tarea')),
            errorEliminar: @json(__('Error al eliminar la tarea')),
            noGuardar: @json(__('No se pudo guardar la tarea')),
            noEliminar: @json(__('No se pudo eliminar la tarea')),
            errorServidor: @json(__('Error de comunicación con el servidor'))
        };
    </script>
