...
<head>
  ...
  <title>{{ __('Tutorial para Padrinos — UGF') }}</title>
...
<body class="rf-page">
  ...
  <header class="rf-tutorial-hero rf-reveal">
    <span class="rf-intro-eyebrow">{{ __('Bienvenido, Padrino') }}</span>
    <h1>{{ __('Tu apoyo cambia el rumbo de un estudiante') }}</h1>
    <p>{{ __('Así es como puedes usar la plataforma UGF para conectar con estudiantes, ofrecer becas y dar seguimiento a tu impacto.') }}</p>
  </header>
  ...
  <main class="rf-tutorial-steps">
    ...
    <article class="rf-card rf-tutorial-card rf-reveal">
      ...
      <h3>{{ __('Crea y ofrece oportunidades de beca') }}</h3>
      <p>{{ __('Publica becas o programas de apoyo financiero, definiendo el monto, los requisitos y el área de estudio que quieres impulsar.') }}</p>
    </article>
    ...
    <article class="rf-card rf-tutorial-card rf-reveal">
      ...
      <h3>{{ __('Explora perfiles de estudiantes') }}</h3>
      <p>{{ __('Revisa los perfiles de estudiantes según su departamento, área de interés y resultado del test socioemocional para encontrar el mejor ajuste.') }}</p>
    </article>
    ...
    <article class="rf-card rf-tutorial-card rf-reveal">
      ...
      <h3>{{ __('Formaliza tu compromiso de apoyo') }}</h3>
      <p>{{ __('Establece un contrato o compromiso claro con el estudiante: monto, duración y condiciones del apoyo, todo dentro de la plataforma.') }}</p>
    </article>
    ...
    <article class="rf-card rf-tutorial-card rf-reveal">
      ...
      <h3>{{ __('Da seguimiento al progreso') }}</h3>
      <p>{{ __('Consulta el avance académico del estudiante que apadrinas y mantente en contacto a través del foro y las notificaciones de la plataforma.') }}</p>
    </article>
    ...
    <article class="rf-card rf-tutorial-card rf-reveal">
      ...
      <h3>{{ __('Participa en el foro') }}</h3>
      <p>{{ __('Conecta con otros padrinos y estudiantes, comparte experiencias y resuelve dudas sobre el proceso de apadrinamiento.') }}</p>
    </article>
    ...
    <article class="rf-card rf-tutorial-card rf-reveal">
      ...
      <h3>{{ __('Consulta tu calendario de becas') }}</h3>
      <p>{{ __('Organiza fechas límite de convocatorias, entrevistas y entregas de fondos desde el calendario integrado de becas.') }}</p>
    </article>

  </main>

  <footer class="rf-tutorial-cta rf-reveal">
    <a href="{{ route('becas.create') }}" class="rf-btn rf-btn-primary">{{ __('Publicar mi primera beca') }}</a>
    <a href="{{ route('perfil') }}" class="rf-btn rf-btn-outline">{{ __('Ir a mi perfil') }}</a>
  </footer>
